...
const EditProfessor: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchProfessor = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/admin/faculty/${facultyId}/professor/${professorId}`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    }
                });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await axios.put(`${import.meta.env.VITE_API_URL}/admin/faculty/${facultyId}/professor/${professorId}`, payload, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <select
                                ...
                                name="materias"
                                multiple
                                className={`w-full h-50 px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 ${errors.subjects ? 'border-red-500' : 'border-gray-300'}`}
